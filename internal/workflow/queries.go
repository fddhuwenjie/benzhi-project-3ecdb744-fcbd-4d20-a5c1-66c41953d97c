...
func (s *Service) Timeline(ctx context.Context, drillID string) ([]audit.Event, error) {
	...
	s.timelineMu.RLock()
	cached, found := s.timelineCache[drillID]
	...
	s.timelineMu.Lock()
	s.timelineCache[drillID] = cloneEvents(events)
	s.timelineMu.Unlock()
	return cloneEvents(events), nil
}

func cloneEvents(events []audit.Event) []audit.Event {
	cloned := make([]audit.Event, len(events))
	copy(cloned, events)
	for index := range cloned {
		cloned[index].Payload = append([]byte(nil), events[index].Payload...)
	}
	return cloned
}
